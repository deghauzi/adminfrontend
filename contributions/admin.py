...
class DailyContributionAdmin(admin.ModelAdmin):
    ...
    list_display = ("TransID","account", "amount", "balance_after_transaction",
    ...
    def get_readonly_fields(self, request, obj=None):
        if obj:
            return ["TransID","transaction_type","account", "amount", "balance_after_transaction",
...
class WalletTransactionAdmin(admin.ModelAdmin):
    ...
    list_display = ("TransID","bank_account_user", "amount", "balance_after_transaction",
                    "transaction_type", "created_by_admin_user")
    list_filter = ("bank_account_user", "amount",
                   "created_by_admin_user", "transaction_type")
    
    def get_readonly_fields(self, request, obj=None):
        if obj:
            return ["TransID","transaction_type","bank_account_user", "amount", "balance_after_transaction",
                                       "created_by_admin_user", "created"]
        else:
            return []
    ...
    def changeform_view(self, request, object_id=None, form_url='', extra_context=None):
        ...
        return super(WalletTransactionAdmin, self).changeform_view(request, object_id, extra_context=extra_context)
    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.transaction_type == 1 and obj.wallet_account.wallet_balance > 0:
            ...
            obj.balance_after_transaction = balance
        if obj.transaction_type == 2:
            obj.balance_after_transaction = obj.wallet_account.wallet_balance - obj.amount
        obj.TransID=f"{gen_key_wa(5)}"
        super().save_model(request, obj, form, change)
...
class TargetContributionAdmin(admin.ModelAdmin):
    list_display = ("TransID","account", "contribution_amount", "contribution_type",
                    "transaction_type", "created_by_admin_user")
    list_filter = ("account", "contribution_amount",
                   "created_by_admin_user", "transaction_type")
    readonly_fields = ["TransID","contribution_balance",
                       "created_by_admin_user", "created"]
    ...
    def changeform_view(self, request, object_id=None, form_url='', extra_context=None):
        ...
        return super(TargetContributionAdmin, self).changeform_view(request, object_id, extra_context=extra_context)
    actions = ["export_as_csv"]
